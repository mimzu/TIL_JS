...
function iconChange(index, name){
    return function(e){
        e.preventDefault() //이건멀ㄲㅏ
        instaMenu[index].children[0].src=`./image/${name}_on.png`
    }
} */
...
/* instaMenu[0].addEventListener('click',function(e){
    e.preventDefault()
    console.log(this)
    this.children[0].src ='./image/like_on.png'
}) */

instaMenu[0].addEventListener('click',function(e){
    e.preventDefault() //이게먼데
...
function iconChange(target, name){
    return target.children[0].src =`./image/${name}_on.png`
}
